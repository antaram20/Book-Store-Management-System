...
BEGIN
	...
	IF nrBooks = 0
	THEN
		INSERT INTO BooksBought(userId, bookId, quantity)
			VALUES (idUser, idBook, 1);
	ELSE
		UPDATE BooksBought SET quantity = quantity + 1
			WHERE userId = idUser
				AND bookId = idBook;
	END IF;
...
CREATE TRIGGER updateStock1
	AFTER UPDATE ON BooksBought FOR EACH ROW
BEGIN
...
CREATE TRIGGER updateStock2
	AFTER INSERT ON BooksBought FOR EACH ROW
BEGIN
